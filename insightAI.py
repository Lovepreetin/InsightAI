...
def generate_pdf_report(perf_df, best_model, problem_type):
    pdf = FPDF()
    pdf.add_page()

    pdf.set_font("Times", 'B', 20) 
    pdf.cell(200, 12, txt="InsightAI - Model Performance Report", ln=True, align='C')
    pdf.ln(10)

    pdf.set_font("Times", '', 12)  
    pdf.cell(200, 8, txt=f"Generated on: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=True)
    pdf.cell(200, 8, txt=f"Problem Type: {problem_type}", ln=True)
    pdf.cell(200, 8, txt=f"Best Model: {best_model}", ln=True)
    pdf.ln(10)

    pdf.set_font("Times", 'B', 12)
    col_width = pdf.w / (len(perf_df.columns) + 1) 
    row_height = 8
    ...
    pdf.cell(0, 10, txt="Generated by InsightAI - Automated ML Platform", ln=True, align='C')
